...
> **Tip:** `~/ltx2` expands to `/home/your-username/ltx2` — no sudo needed.
> This is your personal directory and you have full read/write access to it.
> You can rename it to anything you like, e.g. `~/projects/ltx2`.
...
cd ~/ltx2/LTX-2-Gradio-Implementation
...
crontab -e
# Add: 0 * * * * find ~/ltx2/LTX-2-Gradio-Implementation/outputs/ -name "*.mp4" -mmin +1440 -delete
```
...
> **Note:** This step requires `sudo`. If you do not have sudo access on your
> cloud instance, the tmux method above works just as well for most deployments.

Systemd does **not** expand `~` in service files — you must use the full
absolute path. First, find your home directory:

```bash
echo $HOME
# Example output: /home/ubuntu   (use YOUR actual output below)
```

Create the service file:

```bash
sudo nano /etc/systemd/system/ltx2server.service
```

Paste this — **replace every `/home/ubuntu` with your actual `$HOME` path**:
...
Type=simple
User=ubuntu
WorkingDirectory=/home/ubuntu/ltx2/LTX-2-Gradio-Implementation
Environment="PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True"
Environment="HF_TOKEN=hf_your_token_here"
Environment="MAX_CONCURRENT_GENS=4"
Environment="MAX_CPU_TASKS=2"
ExecStart=/home/ubuntu/ltx2/LTX-2-Gradio-Implementation/.venv/bin/uvicorn server:app --host 0.0.0.0 --port 8000
Restart=on-failure
RestartSec=10

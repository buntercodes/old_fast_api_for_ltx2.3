...
class BlackwellDistilledPipeline(DistilledPipeline):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._text_encoder = None   # ← CRITICAL: cache Gemma to prevent fragmentation OOM
    ...
    def __call__(
        ...
        with torch.no_grad():
            ...
            # Cache text encoder (Gemma) on first call.
            # CRITICAL: Without this, ModelLedger reloads Gemma (~8 GB) from
            # disk on every request, causing VRAM fragmentation and OOM errors
            # even when total free VRAM is 93+ GB (as seen in production logs).
            if self._text_encoder is None:
                logger.info("Caching Gemma text encoder into VRAM (first request)...")
                self._text_encoder = self.model_ledger.text_encoder()
                logger.info("Gemma text encoder cached in VRAM ✅")
...
def load_pipeline(config_req: SystemLoadRequest) -> str:
    ...
    if torch.cuda.is_available():
        ...
        logger.info(f"  Note                 : Gemma text encoder loads into VRAM on first")
        logger.info(f"                         generation request and stays cached permanently.")
        logger.info(f"                         Expect ~8 GB additional VRAM used after request 1.")
...
async def process_generation_task(task: TaskState) -> None:
    ...
    if request.image_base64:
        try:
            ...
            # Fix missing base64 padding — clients often strip trailing '=' characters.
            # Python's b64decode requires padding to be a multiple of 4.
